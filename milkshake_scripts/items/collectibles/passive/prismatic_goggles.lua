...
local DIFFRACTION_LUCK_INCREASE = 0.05
...
local REFLECTION_ALPHA = 0.3
local REFLECTION_BRIGHTNESS = 0.2

local REFLECTION_SWING_SPEED = 0.1
local REFLECTION_AMPLITUDE = 15
...
local function ApplyDiffraction(player)
    ...
    local currentCap = 0
    local chanceMultiplier = math.max(1, player.Luck + player:GetCollectibleNum(PRISMATIC_GOGGLES))
    local diffractionChance = BASE_DIFFRACTION_CHANCE + DIFFRACTION_LUCK_INCREASE * chanceMultiplier
...
function prismaticGoggles:DiffractionRender(npc)
    ...
    for index, tint in ipairs(TINTS) do
        ...
        sprite.Color = Color(tint[1], tint[2], tint[3], REFLECTION_ALPHA, REFLECTION_BRIGHTNESS, REFLECTION_BRIGHTNESS, REFLECTION_BRIGHTNESS)
