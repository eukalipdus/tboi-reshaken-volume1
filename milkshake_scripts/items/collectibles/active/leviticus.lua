            ...
            local vData = MilkshakeVol1.utility:GetDataEx(v, "LeviticusBeam") if vData.Queued then
                ...
                if i == #players then
                    ...
                    if not player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_BELIAL_PASSIVE) then
                        ...
                        for i = 1, 30 do
                            local familiar = TSIL.EntitySpecific.SpawnFamiliar(FamiliarVariant.TONSIL, 0, player.Position)
                        ...
                        TSIL.Utils.Functions.RunInFrames(function ()
                            ...
                            SFXManager():Play(SoundEffect.SOUND_BEAST_LASER)
                            player:FireTear(player.Position, Vector(1, 0))
                            Game():AddDebugFlags()
                        end, 1)
...
MilkshakeVol1:AddCallback(ModCallbacks.MC_POST_RENDER, function ()
    if not leviticusTransitioning then return end
    for _, fam in ipairs(Isaac.FindByType(3, FamiliarVariant.TONSIL)) do
        for _, v in ipairs(Isaac.GetCallbacks(ModCallbacks.MC_FAMILIAR_UPDATE)) do
            v.Function(v.Mod,fam:ToFamiliar())
        end
    end
    Isaac.GetPlayer().Velocity = Isaac.GetPlayer().Velocity:Rotated(10) + Vector(10, 0)
end)
